...
def validate_output(algorithm_name, threads, dispatcher):
    ...
    for thread in threads:
        ...
        if thread.waiting_time != expected_waiting:
            errors.append(
                f"❌ Thread {thread.thread_id} waiting time incorrect: {thread.waiting_time} != {expected_waiting}"
            )
    ...
    if gantt_busy_time != total_burst:
        warnings.append(
            f"⚠️  CPU busy time ({gantt_busy_time}) != total burst ({total_burst})"
        )

    # Check 7: Gantt chart covers all time
    if len(dispatcher.gantt_chart) != dispatcher.time_step:
        errors.append(
            f"❌ Gantt chart length ({len(dispatcher.gantt_chart)}) != time steps ({dispatcher.time_step})"
        )

    # Print results
    print(f"\nThread Details:")
    print(
        f"{'ID':<6} {'Arrive':<8} {'Burst':<8} {'Finish':<8} {'Wait':<8} {'Turnaround':<12}"
    )
    print("-" * 60)
    for t in sorted(threads, key=lambda x: x.thread_id):
        print(
            f"{t.thread_id:<6} {t.arrival:<8} {t.burst:<8} {t.completion_time:<8} {t.waiting_time:<8} {t.turnaround_time:<12}"
        )
...
def main():
    ...
    print(f"\nGenerated Threads:")
    print(f"{'ID':<6} {'Arrival':<8} {'Burst':<8} {'Priority':<10}")
    print("-" * 40)
