...
def validate_results(threads, gantt_chart, algorithm_name):
    ...
    for thread in threads:
        if thread.waiting_time < 0:
            errors.append(f"Thread {thread.thread_id}: negative waiting time ({thread.waiting_time})")
    
    # Check 5: Turnaround = Waiting + Burst
    for thread in threads:
        expected = thread.waiting_time + thread.burst
        if thread.turnaround_time != expected:
            errors.append(f"Thread {thread.thread_id}: turnaround ({thread.turnaround_time}) != waiting ({thread.waiting_time}) + burst ({thread.burst})")
...
def run_algorithm(algorithm_name, algorithm, threads):
    ...
    print("\nThread Results:")
    print(f"{'Thread':<10}{'Arrive':<10}{'Burst':<10}{'Finish':<10}{'Waiting':<10}{'Turnaround':<10}")
    print("-" * 60)
    for t in sorted(threads, key=lambda x: x.thread_id):
        print(f"{t.thread_id:<10}{t.arrival:<10}{t.burst:<10}{t.completion_time:<10}{t.waiting_time:<10}{t.turnaround_time:<10}")
...
def main():
    ...
    for name, algorithm in algorithms:
        ...
        thread_copies = [
            Thread(t.thread_id, t.arrival, t.burst, t.priority)
            for t in threads
        ]
